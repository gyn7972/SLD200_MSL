...
{
    public partial class FormWorkStageParameterMaint : FormSubContentBase
    {
        #region Field
        ...
        #region Constructor
        public FormWorkStageParameterMaint(Module module)
             : base(FormType.Maint.ToString(), module.Name)
        {
            ...
            m_WorkStagePosControl = new ModulePositionControl();
            ...
            m_WorkStagePosControl.Location = new Point(m_JogControl.Location.X, m_JogControl.Location.Y + m_JogControl.Size.Height + Configuration.ControlGap * 2);
            //m_DispenserPosControl.SetPositionList(m_Owner.Config.Positions);
            m_WorkStagePosControl.SetPositionList(m_Owner.workStageParameter.Config.ConfigWorkStagePositions);
            //m_WorkStagePosControl.SetPositionList(m_Owner.Config.Positions);
            m_WorkStagePosControl.SetGroupboxName(" Wafer Probe Align Position Control ");
            this.Controls.Add(m_WorkStagePosControl);
            ...
            m_WorkStagePosControl.ButtonClick += OnClickWorkStagePosControlButton;
        }
        #endregion

        #region Event Handler
        private void OnClickWorkStagePosControlButton(ModulePositionControl.ButtonType type)
        {
            if (type == ModulePositionControl.ButtonType.Save)
            {
                //선택된 Position 확인.
                string strPosition = m_WorkStagePosControl.SelectedPosition;
                WorkStageParameter.stWorkStageParam parameter;
                parameter = m_Owner.workStageParameter.GetPositionInformation(strPosition);
                ...
                WorkStage workStage = m_Owner.Owner as WorkStage;
                if (workStage != null)
                {
                    workStage.SaveConfigData();
                }
