...
namespace SLD200_MSL
{
    ...
    public partial class IlluminatorControl : UserControl
    {
            ...
            if(IlluminatorControlButton_Click != null )
            {
                ...
                if (mb.ShowDialog("얼라인 조명값", "현재 조명값을 얼라인 조명값으로 사용하시겠습니까?") == DialogResult.Yes)
                {
                    foreach (IlluminationChannel channel in m_CurrentDataSet.Values)
                    {
                        ...
                        {
                            workStage.Config.ParamConfig.Align_LowerVision_LightValue = channel.Value;
                        }
                        else if (channel.ChannelName == "Upper")
                        {
                            workStage.Config.ParamConfig.Align_UpperVision_LightValue = channel.Value;
                        }
                    }
        ...
        private void IlluminatorControl_VisibleChanged(object sender, EventArgs e)
        {
            ...
            int m_nLightCount = m_CurrentDataSet.Values.Count;

            if (m_nLightCount > 0)
            {
                m_CurrentDataSet.Values[0].Value = workStage.Config.ParamConfig.Align_UpperVision_LightValue;
                m_CurrentDataSet.Values[1].Value = workStage.Config.ParamConfig.Align_LowerVision_LightValue;
