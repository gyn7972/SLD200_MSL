    ...
    public partial class FormScannerCompensatorMaint : FormSubContentBase
    {
        ...
        private void M_BlobSearchResultControl_SearchClick(Control control)
        {
            ...
            { //? 스케일값 제대로 넣어줘야함
                ...
                {
                    PointD converted = new PointD((result.PixelValues[0][1].Value- this.m_Owner.Camera.Resolution.Width / 2) * ((WaferProbeAlign)this.m_Owner.Owner).Scale.X * (((WaferProbeAlign)this.m_Owner.Owner).Scale.InvertedX ? 1 : -1),
                                                  (result.PixelValues[0][2].Value - this.m_Owner.Camera.Resolution.Height / 2) * ((WaferProbeAlign)this.m_Owner.Owner).Scale.Y * (((WaferProbeAlign)this.m_Owner.Owner).Scale.InvertedY ? 1 : -1));
