...
{
    public delegate void CameraBeforeExposeEventHander();
    ...
    public abstract class Camera : Part
    {
        ...
        public event CameraBeforeExposeEventHander OnBeforeExpose;
        ...
        protected virtual int CreateVisionImage(IntPtr pointer, bool bIsGray, out VisionImage image)
        {
            int ret = 0;
            byte[] bytes = null;
            image = null;
            ...
            try
            {
                ...
                {
                    bytes = new byte[(resolution.Width * resolution.Height) * 3];
                    if (this.ImageRotate == ImageRotateInfo.None)
                    {
                        Marshal.Copy(pointer, bytes, 0, bytes.Length);
                    }
                ...
                this.CreateVisionImage(bytes, bIsGray, out image);
        ...
        /// <param name="image"></param>
        /// <returns></returns>
        protected int CreateVisionImage(byte[] buffer, bool bIsGray, out VisionImage image)
        {
            int ret = 0;
            ...
            try
            {
                ...
                else
                {
                    ...
                    image.Header.Width = this.Resolution.Width;
                    image.Header.Height = this.Resolution.Height;
                    image.Header.BufferSize = (this.CameraConfig.Resolution.Width * this.CameraConfig.Resolution.Height) * 3;
                    image.Header.BitsPerPixel = (int)Camera.BitPerPixelInfo.Color24bpp;
                    image.Header.PixelFormat = PixelFormat.Format24bppRgb;
                    image.Header.Stride = (int)((image.Header.Width * image.Header.BitsPerPixel + 7) / 8);
                    image.Header.OwnerName = this.Name;
                }
            }
        ...
        public Task<int> Grab()
        {
            ...
            {
                return GrabSync(Purpose.Processing);
            });
        ...
        public void FireBeforeExpose()
        {
            if(OnBeforeExpose != null)
            {
                OnBeforeExpose();
            }
        }
        ...
        private int ReadoutProcedure(out VisionImage image)
        {
            ...
            try
            {
                ...
                if ((ret = this.OnReadout(out image)) != 0)
                {
                    return ret;
                }
