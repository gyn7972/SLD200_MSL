...
{
    [Serializable]
    public class WorkStageParameterConfig
    {
        ...
        {
            Origin,                             //  원점 위치 (대기 위치)
            Module_Loading,                     //  제품 로딩 위치
            Low_Mag_Camera_Center,              //  저해상도 카메라의 Stage Center 위치
            High_Mag_Camera_Center,             //  고해상도 카메라의 Stage Center 위치
            Processing,                         //  작업 위치 (Scanner Center 위치)
            PowerMeter,                         //  파워미터 측정 위치
            Scanner_Calibration,                //  스캐너 보정 위치
            High_Mag_Camera_ReticleGlass,       //  고해상도 카메라가 Reticle Glass 를 보는 위치
            Module_Unloading,                   //  제품 언로딩 위치
            Safety,                             //  안전 위치


            //Ready,                              //  대기 위치
            //Jig_Change,                         //  Jig 교체 위치
            //Product_Load,                       //  제품 로딩 위치
            //Product_UnLoad,                     //  제품 언로딩 위치
            //Scanner_Lens_Cleaning,              //  스캐너 렌즈 청소 위치
            //Scanner_StageCenter,                //  스캐너의 스테이지 중앙 위치
            //Scanner_PowerMeter,                 //  스캐너의 파워 미터 위치
            //HighResCamera_StageCenter,          //  고해상도 카메라의 스테이지 중앙 위치
            //HighResCamera_CalSheet_LeftTop,     //  고해상도 카메라의 Calibration Sheet 좌상단 위치
            //HighResCamera_ReticleGlass,         //  고해상도 카메라의 Reticle Glass 위치
            //LowResCamera_StageCenter,           //  저해상도 카메라의 스테이지 중앙 위치
            //LowResCamera_CalSheet_LeftTop,      //  저해상도 카메라의 Calibration Sheet 좌상단 위치
            //LowResCamera_ReticleGlass,          //  저해상도 카메라의 Reticle Glass 위치
            //HeightSensor_StageCenter,           //  높이 센서의 스테이지 중앙 위치
            //HeightSensor_CalSheet_LeftTop,      //  높이 센서의 Calibration Sheet 좌상단 위치
            //MaskPos_Empty,                      //  마스크가 없는 상태
            //MaskPos_1,                          //  마스크 위치 1
            //MaskPos_2,                          //  마스크 위치 2
            //MaskPos_3,                          //  마스크 위치 3
            //MaskPos_4,                          //  마스크 위치 4
        ...
        public XyzyPositionDataCollection WorkStagePositions { set; get; }

        #region Laser/ Scanner Parameter
        [Browsable(false)]
        public List<XyzyPositionData> ConfigWorkStagePositions
        {
            get { return m_WorkStagePosition; }
            set { m_WorkStagePosition = value; }
        }
        ...
        #region Constructor
        public WorkStageParameterConfig()
        {
            Init();
        ...
        protected void Init()
        {
            if (WorkStagePositions == null)
                WorkStagePositions = new XyzyPositionDataCollection();

            m_WorkStagePosition = new List<XyzyPositionData>();
            m_WorkStagePosition.Clear();

            WorkStagePositions.Clear();
            ...
            foreach (PositionLaser key in Enum.GetValues(typeof(PositionLaser)))
            {
                ...
                m_WorkStagePosition.Add(positionBase);
                WorkStagePositions.Add(positionBase);
                ...
                m_WorkStagePosition.Add(positionTarget);
                WorkStagePositions.Add(positionTarget);
            }
        }
