        ...
        public stUnloaderParam GetPositionInformation(string m_strPosName)
        {
            ...
            for (int i = 0; i < parameters.Count; i++)
            {
                if (parameters[i].Name == m_strPosName)
                {
                    ...
                    m_stUnloaderPosParam.dTarget[0] = m_dPos[0] = Equipment.ToDouble(parameters[i++].Value);       //  Z0
                    m_stUnloaderPosParam.dTarget[1] = m_dPos[1] = Equipment.ToDouble(parameters[i++].Value);       //  Z1
                    m_stUnloaderPosParam.dTarget[2] = m_dPos[2] = Equipment.ToDouble(parameters[i++].Value);       //  TR_X
                    m_stUnloaderPosParam.dTarget[3] = m_dPos[3] = Equipment.ToDouble(parameters[i++].Value);       //  TR_Z

                    //  Offset 값도 가져오자.
                    nPosCnt = 0;
                    m_stUnloaderPosParam.dOffset[0] = Equipment.ToDouble(parameters[i++].Value);                   //  Z0
                    m_stUnloaderPosParam.dOffset[1] = Equipment.ToDouble(parameters[i++].Value);                   //  Z1
                    m_stUnloaderPosParam.dOffset[2] = Equipment.ToDouble(parameters[i++].Value);                   //  TR_X
                    m_stUnloaderPosParam.dOffset[3] = Equipment.ToDouble(parameters[i++].Value);                   //  TR_Z
