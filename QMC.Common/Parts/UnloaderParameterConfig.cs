...
{
    [Serializable]
    public class UnloaderParameterConfig
    {
        protected List<ZzxzPositionData> m_UnloaderPosition;

        //  Unloader 장비에서 사용되는 위치
        public enum PositionUnloader
        {
            ...
            //Ready,                              //  대기 위치
            //Picker_MovablePos_Z,                //  Unloader 가 충돌 없이 움직일 수 있는 위치 (Z축)
            //Picker_PNL_PickUp,                  //  PNL 을 집어올리는 위치 (Work Table)
            //Picker_Stack0_PutDown_Ready,        //  Stacker0 에서 제품 PutDown 대기 위치
            //Picker_Stack0_PutDown,              //  Stacker0 에서 제품 PutDown 위치
            //Picker_Stack1_PutDown_Ready,        //  Stacker1 에서 제품 PutDown 대기 위치
            //Picker_Stack1_PutDown,              //  Stacker1 에서 제품 PutDown 위치
            ...
            //NG_Drop,                            //  불량 제품을 내려놓는 위치
        }

        public ZzxzPositionDataCollection UnloaderPositions { set; get; }

        #region Laser/ Scanner Parameter
        [Browsable(false)]
        public List<ZzxzPositionData> ConfigUnloaderPositions
        {
            get { return m_UnloaderPosition; }
            set { m_UnloaderPosition = value; }
        }
        ...
        #region Constructor
        public UnloaderParameterConfig()
        {
            Init();
        ...
        protected void Init()
        {
            if (UnloaderPositions == null)
                UnloaderPositions = new ZzxzPositionDataCollection();

            m_UnloaderPosition = new List<ZzxzPositionData>();
            m_UnloaderPosition.Clear();

            UnloaderPositions.Clear();



            foreach (PositionUnloader key in Enum.GetValues(typeof(PositionUnloader)))
            {
                ZzxzPositionData positionBase = new ZzxzPositionData();

                positionBase.Name = key.ToString();
                m_UnloaderPosition.Add(positionBase);
                UnloaderPositions.Add(positionBase);
                ...
                m_UnloaderPosition.Add(positionTarget);
                UnloaderPositions.Add(positionTarget);
            }
        }
        ...
        public List<string> GetUnloaderPositionList()
        {
            ...
            foreach (ZzxzPositionData position in m_UnloaderPosition)
            {
        ...
        {
            int i = nStartIndex;
            foreach (ZzxzPositionData pos in m_UnloaderPosition)
            {
                //  Loader Stacker Z0
