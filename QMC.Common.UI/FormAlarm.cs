...
namespace QMC.Common.UI
{
    public partial class FormAlarm : FormSubContentBase
    {
...

        {
            ...
            m_BaseButton = new BaseButton();
            m_BaseButton.Text = "Comfirm";
            m_BaseButton.Size = Configuration.ButtonSize;
            m_BaseButton.TextAlign = ContentAlignment.MiddleCenter;
            m_BaseButton.FlatStyle = FlatStyle.Flat;
            m_BaseButton.Click += ButtonComfirm_Click;

            this.panelComfirm.Controls.Add(m_BaseButton);

            CommonModule.Instance.OperationButtons.ButtonClick += OnOperationButtonClicked;
        }
        ...
        private void FormAlarm_VisibleChanged(object sender, EventArgs e)
        {
            ...
            if (Alarms != null && Alarms.Count > 0)
            {
                ...
                for (int i = 0; i < Alarms.Count; i++)
                {
                    ...
                    {
                        Image img = Properties.Resources.AlarmInform;
                    ...
                    {
                        Image img = Properties.Resources.AlarmError;
                        Bitmap imgbitmap = new Bitmap(img);
                        img = resizeImage(imgbitmap, m_imagesize);
                        Alarms[i].StateImage = img;
                    }
        ...
        private void OnOperationButtonClicked(OperationButtons.ButtonType type)
        {
            if (type == OperationButtons.ButtonType.Reset)
            {
