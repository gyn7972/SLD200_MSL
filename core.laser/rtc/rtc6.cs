...
namespace QMC.Core.Laser
{
    ...
    // SLO 310 Scanner Spec
    // typical image field size :: 45 x 45 mm
    // max beam diameter specified : 9 mm
    // wavelenth 266 
        ...
        : IRtc
    {
        ...
        /// 리스트당 명령을 허용할 개수 제한값
        /// </summary>
        protected const uint RTC6_LIST_BUFFER_MAX = 4000;

        protected float frequency; //hz
        protected float pulseWidth;//usec
        ...
        public Rtc6(uint index, string name)
        {
            this.Index = index;
            this.Name = name;
            this.KFactor = 0.0;
        ...
        /// <returns></returns>
        public bool Initialize(double kFactor, string correctionFileName, 
            LaserMode laserMode = LaserMode.Yag1, SignalLevel signalLevel = SignalLevel.ActiveHigh)
        {
            ...
            RTC6Wrap.n_stop_execution(this.Index);
            uint error = RTC6Wrap.n_load_program_file(this.Index, string.Empty);
            if (0 != error)
                return false;
            uint lastError = RTC6Wrap.n_get_last_error(this.Index);
            if (0 != lastError)
                RTC6Wrap.n_reset_error(this.Index, lastError);
            ...
            uint serialNo = RTC6Wrap.n_get_serial_number(this.Index);
            ...
            switch (signalLevel)
            {
                case SignalLevel.ActiveHigh:
                    RTC6Wrap.n_set_laser_control(this.Index, 0);
                    break;
                case SignalLevel.ActiveLow:
                    RTC6Wrap.n_set_laser_control(this.Index, (0x01 << 3) | (0x01 << 4));
                    break;
            }
            // laser mode
            RTC6Wrap.n_set_laser_mode(this.Index, (uint)laserMode);
            // fpk to 0
            RTC6Wrap.n_set_firstpulse_killer(this.Index, 0);
            // stand by timing to 0
            RTC6Wrap.n_set_standby(this.Index, 0, 0);
            // maximum size of list 1 and 2
            //RTC6Wrap.n_config_list(this.Index, RTC6_LIST_BUFFER_MAX * 2, RTC6_LIST_BUFFER_MAX * 2);
            RTC6Wrap.n_config_list(this.Index, (uint)Math.Pow(2, 20), 0);
        ...
        public bool CtlLoadCorrectionFile(string correctionFileName)
        {
            ...
            string fullPathName = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "correction", correctionFileName);
            uint error = RTC6Wrap.n_load_correction_file(this.Index, fullPathName, 1, 2);
            if (0 != error)
            {
                switch (error)
                {
            ...
            RTC6Wrap.n_select_cor_table(this.Index, 1, 0);
        ...
        {
            RTC6Wrap.n_laser_signal_on(this.Index);
            return true;
        }
        /// <summary>
        /// 수동 레이저 출사 정지
        ...
        {
            RTC6Wrap.n_laser_signal_off(this.Index);
            return true;
        }
        /// <summary>
        /// 스캐너 위치 이동
        ...
        public bool CtlMove(double x, double y)
        {
            ...
            RTC6Wrap.n_goto_xy(this.Index, (int)xBits, (int)yBits);
            return true;
        }
        /// <summary>
        /// 레이저 타이밍(주파수, 펄스폭) 설정
        ...
        public bool CtlFrequency(double frequency, double pulseWidth)
        {
            ...
            RTC6Wrap.n_set_start_list(this.Index, 1);
            RTC6Wrap.n_set_laser_timing(this.Index,
                (uint)(halfPeriod * 64.0),
                (uint)(pulseWidth * 64.0),
                0,
                0);
            RTC6Wrap.n_set_end_of_list(this.Index);
            RTC6Wrap.n_execute_list(this.Index, 1);
            this.CtlBusyWait();
            return true;
        }
        /// <summary>
        ...
        public bool CtlDelay(double laserOn, double laserOff, double scannerJump, double scannerMark, double scannerPolygon)
        {
            if (this.CtlGetStatus(RtcStatus.Busy))
                return false;
            RTC6Wrap.n_set_start_list(this.Index, 1);
            RTC6Wrap.n_set_scanner_delays(this.Index,
                (uint)(scannerJump / 10.0),
                (uint)(scannerMark / 10.0),
                (uint)(scannerPolygon / 10.0)
                );
            RTC6Wrap.n_set_laser_delays(this.Index,
                (int)(laserOn * 64.0),
                (uint)(laserOff * 64.0)
                );
            RTC6Wrap.n_set_end_of_list(this.Index);
        ...
        public bool CtlSpeed(double jump, double mark)
        {
            ...
            RTC6Wrap.n_set_jump_speed_ctrl(this.Index, jump_bitperms);
            RTC6Wrap.n_set_mark_speed_ctrl(this.Index, mark_bitperms);
            return true;
        }
        /// <summary>
        /// RTC 확장채널에 데이타 쓰기
        ...
        public bool CtlWriteData<T>(ExtensionChannel ch, T value)
        {
            switch (ch)
            {
                ...
                case ExtensionChannel.ExtAO1:
                    ...
                    RTC6Wrap.n_write_da_x(this.Index, 1, a1Value);
                    break;
                case ExtensionChannel.ExtAO2:
                    ...
                    RTC6Wrap.n_write_da_x(this.Index, 2, a2Value);
                    break;
                default:
                    return false;
            }
        ...
        public bool CtlGetStatus(RtcStatus status)
        {
            ...
            {
                case RtcStatus.Busy:
                    RTC6Wrap.n_get_status(this.Index, out busy, out position);
                ...
                case RtcStatus.List1Busy:
                    uint l1Status = RTC6Wrap.n_read_status(this.Index);
                ...
                case RtcStatus.PositionAckOK:
                    uint posAckStatus = RTC6Wrap.n_get_head_status(this.Index, 1);
                ...
                case RtcStatus.PowerOK:
                    uint powStatus = RTC6Wrap.n_get_head_status(this.Index, 1);
                    result = Convert.ToBoolean(powStatus & (0x01 << 7));
                    break;
                case RtcStatus.TempOK:
                    uint tempStatus = RTC6Wrap.n_get_head_status(this.Index, 1);
        ...
        protected string CtlGetErrMsg(uint errorCode)
        {
            if (0 == errorCode)
                return string.Empty;
            uint code = RTC6Wrap.n_get_error(this.Index);
        ...
        public bool ListBegin()
        {
            ...
            this.listIndex = 1;
            this.listCount = 0;
            RTC6Wrap.n_set_start_list(this.Index, this.listIndex);
            return true;
        }
        /// <summary>
        ...
        //        {
        //            RTC6Wrap.n_set_end_of_list(this.Index);
        //            RTC6Wrap.n_execute_list(this.Index, this.listIndex);
        ...
        //            RTC6Wrap.n_auto_change(this.Index);
        //            uint readStatus = 0;
        //            switch (this.listIndex)
        //            {
        //                case 1:
        //                    do {
        //                        readStatus = RTC6Wrap.n_read_status(this.Index);
        //                        System.Threading.Thread.Sleep(1);
        ...
        public bool ListFrequency(double frequency, double pulseWidth)
        {
            ...
            //    return false;
            //RTC6Wrap.n_list_nop(this.Index);
            RTC6Wrap.n_set_laser_timing(this.Index,
                (uint)(halfPeriod * 64.0),
                (uint)(pulseWidth * 64.0),
                0, 0);
        ...
        public bool ListSpeed(double jump, double mark)
        {
            ...
            //    return false;
            //RTC6Wrap.n_list_nop(this.Index);
            RTC6Wrap.n_set_jump_speed(this.Index, jump_bitperms);
            RTC6Wrap.n_set_mark_speed(this.Index, mark_bitperms);
            return true;
        }        
        /// <summary>
        ...
        public bool ListJumpTo(double x, double y, double weight = 1.0)
        {
            ...
            //    return false;
            //RTC6Wrap.n_jump_abs(this.Index, (int)xBits, (int)yBits);
            RTC6Wrap.n_para_jump_abs(this.Index, xBits, yBits, (uint)(weight * this.powerXValue));
            return true;
        }
        /// <summary>
        ...
        public bool ListMarkTo(double x, double y, double weight = 1.0)
        {
            ...
            //if (!this.IsListBufferReady(1))
            //    return false;
            //RTC6Wrap.n_mark_abs(this.Index, (int)xBitsOut, (int)yBitsOut);
            RTC6Wrap.n_para_mark_abs(this.Index, xBits, yBits, (uint)(weight * this.powerXValue));
            return true;
        }
        /// <summary>
        ...
        public bool ListArc(double cx, double cy, double sweepAngle, double weight = 1.0)
        {
            ...
            //if (!this.IsListBufferReady(1))
            //    return false;
            RTC6Wrap.n_arc_abs(this.Index, xBits, yBits, -sweepAngle);
            return true;
        }
        /// <summary>
        ...
        public bool ListPixelLine(double usec, ExtensionChannel ext, double dx, double dy, uint pixelCount)
        {
            ...
            {
                case ExtensionChannel.ExtAO1:
                    RTC6Wrap.n_set_pixel_line(this.Index, 1, (uint)(usec / 2.0 * 64.0), xBits, yBits);
                    break;
                case ExtensionChannel.ExtAO2:
                    RTC6Wrap.n_set_pixel_line(this.Index, 2, (uint)(usec / 2.0 * 64.0), xBits, yBits);
                    break;
                default:
                    RTC6Wrap.n_set_pixel_line(this.Index, 0, (uint)(usec / 2.0 * 64.0), xBits, yBits);
                    break;
            }
            return true;
        }
        ...
        public bool ListPixel(double usec, double voltage = 0.0f)
        {
            ...
            RTC6Wrap.n_set_pixel(this.Index, (uint)(usec * 64.0), value);
            return true;
        }
        /// <summary>
        /// RTC 확장채널에 데이타 쓰기
        ...
        public bool ListWriteData<T>(ExtensionChannel ch, T value)
        {
            ...
            switch (ch)
            {
                case ExtensionChannel.ExtDO16:
                    uint d16Out = (uint)Convert.ChangeType(value, typeof(uint));
                    //RTC6Wrap.n_write_io_port_mask_list(this.Index, ???);
                    RTC6Wrap.n_write_io_port_list(this.Index, d16Out);
                    break;
                case ExtensionChannel.ExtDI8O8:
                    uint d8Out = (uint)Convert.ChangeType(value, typeof(uint));
                    RTC6Wrap.n_write_8bit_port_list(this.Index, d8Out);
                    break;
                case ExtensionChannel.ExtDI2O2:
                    uint d2Out = (uint)Convert.ChangeType(value, typeof(uint));
                    RTC6Wrap.n_set_laser_pin_out_list(this.Index, d2Out);
                    break;
                case ExtensionChannel.ExtAO1:
                    double ao1 = (double)Convert.ChangeType(value, typeof(double));
                    double ao1_bits = (Math.Pow(2, 12) - 1) * ao1 / 10.0;
                    RTC6Wrap.n_write_da_x_list(this.Index, 1, (uint)ao1_bits);
                    break;
                case ExtensionChannel.ExtAO2:
                    double ao2 = (double)Convert.ChangeType(value, typeof(double));
                    double ao2_bits = (Math.Pow(2, 12) - 1) * ao2 / 10.0;
                    RTC6Wrap.n_write_da_x_list(this.Index, 2, (uint)ao2_bits);
                    break;
                default:
                    return false;
            }
        ...
        public bool ListLaserControl(PowerXFactor powerXFactor, double powerXValue)
        {
            ...
            switch (powerXFactor)
            {
                ...
                case PowerXFactor.FocusShift:
                    ...
                    //    RTC6Wrap.n_set_vector_control(this.Index, (uint)laser.PowerXFactor, (uint)zDistanceBits);
                    //    //none ? success &= ListJump(0,0, powerX); //offset ?
                    //    this.powerXValue  = (float)zDistanceBits;
                    success = false;
                    break;
            }
            this.powerXFactor = powerXFactor;
        ...
        {
            double maxsize = 14; 
            double pitchX = maxsize / (cntx - 1);
            double pitchY = maxsize / (cnty - 1);
            double xsize = 0.2;
            ...
            var t = Task.Run(() =>
            {
                ...
                {
                    ListJumpTo(-maxsize / 2 - 1, -(pitchY * (cnty - 1) / 2) + pitchY * i);
                    ListMarkTo(maxsize / 2 + 1, -(pitchY * (cnty - 1) / 2) + pitchY * i);

                    ListJumpTo(-(pitchX * (cntx - 1) / 2) + pitchX * i, -maxsize / 2 - 1);
                    ListMarkTo(-(pitchX * (cntx - 1) / 2) + pitchX * i, maxsize / 2 + 1);
                }
                //-----------------------------------------------------------------------------------------------------------------------
                ...
                //    {
                //        //DrawMarkX(-maxsize / 2 + (pitchX * k), -maxsize / 2 + (pitchY * j), 1);
                //        this.ListJumpTo(-maxsize / 2 + (pitchX * k) + (xsize / 2), -maxsize / 2 + (pitchY * j) + (xsize / 2));
                //        this.ListMarkTo(-maxsize / 2 + (pitchX * k) - (xsize / 2), -maxsize / 2 + (pitchY * j) - (xsize / 2));
                ...
                //    for (int k = 0; k < cnty; k++)
                //    {
                //        this.ListJumpTo(-maxsize / 2 + (pitchX * k) - 10 * 0.1, -maxsize / 2 + (pitchY * j));
                //        ListPixelLine(500, ExtensionChannel.ExtAO1, 0.1, 0, 1 + 10);
